...
def SCTransform(adata, min_cells=0, gmean_eps=1, n_genes=2000, n_cells=None, bin_size=500, bw_adjust=3, inplace=True):
    ...
    # we would like to use the mean value for our downstream augmentations.
    # Use the mud_X to store the regressed values
    mud_X = X.copy()
    ...
    for i in range(1, int(max_bin) + 1):
        ...
        with get_context("spawn").Pool(os.cpu_count(), _parallel_init,
                                       [genes_bin_regress, umi_bin, gn, mm, ps]) as pool:
            try:
    ...
    if inplace:
        ...
        # if inplace = Flase, we create a anndata, where the count data is the regressed values
        adata_new = AnnData(X=mud_X)
